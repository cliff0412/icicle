...
int main(int argc, char* argv[])
{
  ...
  scalar_t* elements;
  elements = (scalar_t*)malloc(sizeof(scalar_t) * batch_size);

  scalar_t* output;
  output = (scalar_t*)malloc(sizeof(scalar_t) * batch_size);
  ...
  ntt::NTTConfig<scalar_t> config = ntt::DefaultNTTConfig<scalar_t>();

  // ntt_end2end_batch_template<scalar_t, scalar_t>(elements, batch_size, ntt_size, inverse, stream);
  ntt::NTT(elements, ntt_size, ntt::NTTDir::kForward, config, output);
